        ...
        if phone_number := options['phone_number']:
            ...
            if options['func'] is not None:
                ...
                for i in range(1, HOST_FUNC_COUNT + 1):
                    ...
                    if os.name == 'posix':
                        os.system(f'nohup {BASE_DIR}/venv/bin/python manage.py start {phone_number} --host {options["host"]} --func {i} &')
                    elif os.name == 'nt':
                        os.system(f'start /B {BASE_DIR}/venv/Scripts/python.exe manage.py start {phone_number} --host {options["host"]} --func {i}')
                    sleep(1)
            return
        if os.name == 'posix':
            for i, phone_number in enumerate(USERBOT_HOST_LIST, 1):
                os.system(f'nohup {BASE_DIR}/venv/bin/python manage.py start {phone_number} --host {i} &')
                sleep(1)
            for phone_number in USERBOT_PN_LIST:
                os.system(f'nohup {BASE_DIR}/venv/bin/python manage.py start {phone_number} &')
                sleep(1)
        elif os.name == 'nt':
            for i, phone_number in enumerate(USERBOT_HOST_LIST, 1):
                os.system(f'start /B {BASE_DIR}/venv/Scripts/python.exe manage.py start {phone_number} --host {i}')
                sleep(1)
            for phone_number in USERBOT_PN_LIST:
                os.system(f'start /B {BASE_DIR}/venv/Scripts/python.exe manage.py start {phone_number}')
                sleep(1)
